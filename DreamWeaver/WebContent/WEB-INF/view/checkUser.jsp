...
$(document).ready(function(){
	...
	function a(a){
		if(a=='f'){//首页
			location.href="checkUser?count="+count+"&nowpage="+0;	
		}else if(a=='l'){//前一页
			if(np==1){//如果是第一页
				alert("亲,已经是第一页了");
			}else{
		np=np-2;
		location.href="checkUser?count="+count+"&nowpage="+np;		
			}
		}else if(a=='n'){//下一页
		if(np==pc){
			alert("亲,已经是最后一页了");
			...
			pc=pc-1;
			location.href="checkUser?count="+count+"&nowpage="+pc;
		}
		}
	//编号查询
	$("#codebt").click(function(){
		...
		location.href="checkUser?code="+$("#code").val();
		}
	});
	//姓名查询
	$("#namebt").click(function(){
		...
		location.href="checkUser?name="+$("#name").val();
		}
	});
	//翻页按钮设置监听
	$("#f").click(function(){
    ...
    <td height="35">
      ...
      <tr>
        ...
        <td background="images/tab_05.gif">
          ...
          <span class="STYLE3">&nbsp;我&nbsp;的&nbsp;用&nbsp;户&nbsp;列&nbsp;表</span><span >(${lvmsg})</span>
        </td>
      ...
      <tr>
        ...
        <td bgcolor="#e5f1d6">
          ...
          <s:iterator value="users" status="sta" var="user">
          	<s:if test="#user.status==1"><!-- 如果是激活状态 -->
