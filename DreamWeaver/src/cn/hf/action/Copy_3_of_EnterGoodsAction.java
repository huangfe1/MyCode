...
public class Copy_3_of_EnterGoodsAction implements Action{
	private int type;
				...
				if(type==0){//串号录入
					...
					for(int i=0;i<count;i++){
						...
						if(f<=e){//循环录入数组
							...
							/*	}else{		 //存在,如果产品名一致,是我发出去的货物
									//公司用户与二级管理员可以修改权利可以修改,
									if(user.getType()==0||user.getType()==2){//直接修改
										GoodsDao.changeGoods(goods);
										//插入记录
										GoodsDao.insertHis(1, goods.getUid(), goods.getGid(), goods.getTime());
										wgses.add(gs);//但是加入错误编号
									}else{
